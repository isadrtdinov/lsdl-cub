...
def plot(results_db, train_dataset, eval_dataset, save_path):
    fig, ax = plt.subplots()
    ...
    x = f'{train_dataset}:top1'
    y = f'{eval_dataset}:top1'

    x_range = (0.99 * min(df[x]), 1.01 * max(df[x]))
    y_range = (0.99 * min(df[y]), 1.01 * max(df[y]))

    transform = adjust_plot(ax, x_range, y_range, scaling=None, tick_freq=0.01)
    ...
    ax.plot(
        transform(zeroshot[x]) + transform(wise_ft[x]) + transform(finetuned[x]),
        transform(zeroshot[y]) + transform(wise_ft[y]) + transform(finetuned[y]),
        alpha=0.8, c='C6',
    )

    ax.set_xlabel(f'In-distribution accuracy ({train_dataset})', fontsize=16)
...
if __name__ == '__main__':
    ...
    for eval_dataset in args.eval_datasets:
        ...
        plot(args.results_db, args.train_dataset, eval_dataset, save_path)